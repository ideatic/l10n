...
class JSON extends Serializer
{
    ...
    {
        if ($this->extended) {
            $catalog = $this->_generateExtended($domains);
        } else {
            $catalog = $this->_generateSimple($domains);
        }


        return json_encode($catalog, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_THROW_ON_ERROR | JSON_UNESCAPED_SLASHES);
    }

    /**
     * @param Domain[] $domains
     */
    protected function _generateSimple(array $domains): array
    {
        $translations = [];
            ...
            foreach ($domain->strings as $strings) {
                ...
                if ($this->locale) {
                    ...
                    if (isset($translation)) {
                        $translations[$stringID] = $translation->translation;
                    }
                } else {
     ...
     */
    public function _generateExtended(array $domains): array
    {
        $translations = [];
